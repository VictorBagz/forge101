  ...
  const navItems = [
    { name: 'HOME', href: '/', active: true },
    {
      ...
      dropdown: [
        ...
        { name: 'Training Methods', href: '/training-methods' },
    ...
    },
    {
      ...
      dropdown: [
        { name: 'Adult-Fitness', href: '/adult-fitness' },
        { name: 'Elite Athletes', href: '/elite-athletes' },
        { name: 'Injured Athletes', href: '/injured-athletes' },
        { name: 'Youth Athletes', href: '/youth-athletes' },
      ]
    },
    {
      name: 'WHAT WE DO',
      dropdown: [
        { name: 'Performance Training', href: '/performance-training' },
        { name: 'Adult Fitness', href: '/adult-fitness' },
        { name: 'Sports Conditioning', href: '/sports-conditioning' },
        { name: 'Sports Recovery', href: '/sports-recovery' },
        { name: 'Sports Medicine', href: '/sports-medicine' },
        { name: 'Wellness Tracking', href: '/wellness-tracking' },
        { name: 'Schedule & Calendar', href: '/schedule-calendar' },
        { name: 'Exercise Library', href: '/exercise-library' },
      ]
    },
    {
      name: 'GET STARTED',
      dropdown: [
        { name: 'Find a Location', href: '/find-location' },
        { name: 'Contact Us', href: '#' },
      ]
    },
  ];

  // Helper to handle navigation - supports both routes and hash links
        ...
        <div className="hidden md:flex items-center flex-1 justify-end space-x-8">
          {navItems.map((item) => (
            <div key={item.name} className="relative group h-10 flex items-center">
              {item.dropdown ? (
                <span className={`flex items-center text-sm font-bold uppercase tracking-wider transition-colors cursor-pointer ${item.active ? 'text-forge-accent' : `${textColorClass} ${hoverColorClass}`}`}>
                  {item.name}
                  <ChevronDown size={14} className={`ml-1 transition-all duration-200 group-hover:text-forge-accent group-hover:rotate-180 ${subTextColorClass}`} />
                </span>
              ) : (
                <Link
                  to={item.href || '/'}
          ...
          <div className="md:hidden fixed right-0 top-0 h-screen w-full sm:w-96 bg-white shadow-2xl overflow-y-auto pb-32 z-50 animate-slideInRight">
            <div className="px-6 py-8 space-y-6">
                ...
                <div key={item.name} className="border-b border-gray-100 pb-4 last:border-0">
                  {item.dropdown ? (
                    <>
                      ...
                      >
                        {item.name}
                        <ChevronDown size={20} className={`transition-transform duration-300 ${activeMobileDropdown === item.name ? 'rotate-180 text-forge-accent' : 'text-gray-400'}`} />
                      </button>
              ...
              <div className="pt-4">
                <a
                  href="/#membership"
                  onClick={(e) => handleNavigation('/#membership', e)}
